...
// // // import { envClauseMappings, envComments, envSentimentKeywords, envStats, mockThemes, envSectionSummaries } from "@/data/mockDataSummary";
...
// // //     const positives = [...sentimentKeywords]
// // //       .sort((a, b) => b.positiveCount - a.positiveCount)
// // //       .slice(0, 2)
// // //       .map((k) => k.word);

// // //     const negatives = [...sentimentKeywords]
...
// // //     const stakeholderEntries = Object.entries(stats.stakeholderBreakdown) as Array<[string, number]>;
...
// // //             {Object.entries(stats.overallSentimentDistribution).map(([sentiment, percentage]) => (
...
// // //                 const percentage = stats.stakeholderBreakdown[stakeholder as keyof typeof stats.stakeholderBreakdown];
...
// // //   const sectionData = envSectionSummaries[selectedSection as keyof typeof envSectionSummaries];
...
import { Button } from "@/components/ui/button";
import { FileText, Filter, Search, Sparkles, ListChecks, Building2, User, Heart, Briefcase, GraduationCap, Factory, Store } from "lucide-react";
import { mockClauseMappings as envClauseMappings, mockComments as envComments, mockSentimentKeywords as envSentimentKeywords, mockStats as envStats, mockThemes, mockSectionSummaries as envSectionSummaries } from "@/data/mockDataSummary";
import { mdpClauseMappings, mdpComments, mdpSentimentKeywords, mdpStats, mdpSectionSummaries } from "@/data/mockDataMDP";

// Determine selected draft and datasets
const selectedDraftId = typeof window !== 'undefined' ? localStorage.getItem('selectedDraftId') : null;
const isMDP = selectedDraftId === '2';

const clauseMappings = isMDP ? mdpClauseMappings : envClauseMappings;
const comments = isMDP ? mdpComments : envComments;
const sentimentKeywords = isMDP ? mdpSentimentKeywords : envSentimentKeywords;
const stats = isMDP ? mdpStats : envStats;
const sectionSummaries = isMDP ? mdpSectionSummaries : envSectionSummaries;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {selectedDraftTitle ? (
                <span className="text-lg text-gray-600">
                  Selected draft: <span className="font-medium text-lg text-black">{selectedDraftTitle}</span>
                </span>
              ) : (
                "Concise overview of key consultation insights"
              )}
...
const SectionSummary = () => {
  const isMDPCurrent = typeof window !== 'undefined' ? localStorage.getItem('selectedDraftId') === '2' : false;
  const summaries = isMDPCurrent ? mdpSectionSummaries : envSectionSummaries;
  const sectionKeys = Object.keys(summaries);
  const [selectedSection, setSelectedSection] = useState<string>(sectionKeys[0] ?? "");

  useEffect(() => {
    const keys = Object.keys(summaries);
    if (!keys.includes(selectedSection)) {
      setSelectedSection(keys[0] ?? "");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isMDPCurrent]);

  const sectionData = (summaries as any)[selectedSection];
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="min-w-0">
            <Select value={selectedSection} onValueChange={setSelectedSection}>
              <SelectTrigger className="w-56 sm:w-64 md:w-72 lg:w-80 max-w-full h-12 text-base">
                <SelectValue placeholder="Select section" />
              </SelectTrigger>
              <SelectContent align="end" sideOffset={4} position="popper" className="z-50 w-[var(--radix-select-trigger-width)] max-w-[80vw]">
                {Object.entries(summaries).map(([key, val]) => (
                  <SelectItem key={key} value={key}>{(val as any).title}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
